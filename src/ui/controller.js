...
import { bindCanvasPointerEvents } from './waveform-interactions.js';

export {
  applyRelationEndpointDrop,
  bindCanvasPointerEvents,
  pointerSvgY,
  relationEndpointUpdates,
  resolveDropTransitionId,
  sequenceFromPointer,
  slotWidthFromPointer,
  timingPositionFromPointer
...
function renderSignalLabelRail(documentModel) {
  return `<aside id="signal-label-rail" aria-label="Signal labels"><div class="signal-label-rail-spacer"></div>${signalRowsForCanvas(documentModel)
    .map((signal) => `<div class="signal-label-rail-row" data-signal-label-id="${escapeHtml(signal.id)}"><strong title="${escapeHtml(signal.name)}">${escapeHtml(signal.name)}</strong><span>${escapeHtml(signal.type)}</span></div>`)
    .join('')}</aside>`;
}

function renderCanvasSvg(documentModel, slotWidthUnits) {
...
function historyDisclosureMarkup(history, activeHistoryId) {
  ...
  return `<section class="example-document"><button type="button" class="button secondary" data-open-example>Example document</button><p class="muted">ENVM power-on → write → power-off</p><p class="muted">Open a fresh, editable copy.</p></section><details class="history-disclosure"><summary>Document history <span>${history.entries.length}</span></summary><div class="history-list">${historyItems}</div></details>`;
}
...
export function renderEditorMarkup(documentModel, { mode, validation, view = 'waveform', repairText = '' }) {
  ...
  const content = activeView === 'json'
    ...
    : `<div class="drag-status-slot"><p id="drag-status" class="drag-status" aria-live="polite" hidden></p></div><div class="waveform-shell">${renderSignalLabelRail(documentModel)}<div id="waveform-canvas" tabindex="0" aria-label="Waveform timeline; drag the background to scroll horizontally">${renderCanvasSvg(documentModel)}</div></div>`;
...
export function createEditor(root = document) {
  ...
  function bindCanvasEvents(svg = editor.querySelector('svg')) {
    ...
    bindCanvasPointerEvents(svg, {
      ...
      previewCanvas(slotWidthUnits, pointerId, drag) {
        const canvas = editor.querySelector('#waveform-canvas');
        if (!canvas) return;
        canvas.innerHTML = renderCanvasSvg(state.document, slotWidthUnits);
  ...
  function renderEditor() {
    const scrollLeft = editor.querySelector('#waveform-canvas')?.scrollLeft ?? 0;
    ...
    const canvas = editor.querySelector('#waveform-canvas');
    if (canvas) canvas.scrollLeft = scrollLeft;
    if (state.mode === 'repair') {
  ...
  palette.addEventListener('click', (event) => {
    if (event.target.closest('[data-open-example]')) {
      if (state.mode === 'editor') state.history = replaceActiveHistoryEntry(state.history, state.document);
      state.document = createExampleDocument();
      state.history = appendHistoryEntry(state.history, createHistoryEntry(state.document));
      state.mode = 'editor';
      state.view = 'waveform';
      state.selectedTransitionId = null;
      state.relationCreation = null;
      state.repairSelection = null;
      state.repairText = '';
      state.drag = null;
      const saved = saveHistory(storage, state.history);
      setNotice(saved.saved ? 'Opened a fresh example copy. Your previous document remains in history.' : saved.notice);
      render();
      const canvas = editor.querySelector('#waveform-canvas');
